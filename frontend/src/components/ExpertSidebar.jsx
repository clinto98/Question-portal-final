...
const menuItems = [
  // { name: "Dashboard", path: "/expert/dashboard", icon: HiOutlineViewGrid },
  { name: "Finalize Questions", path: "/expert/finalize", icon: HiOutlineClipboardCheck },
  { name: "Finalized Questions", path: "/expert/finalized", icon: HiOutlineCollection },
];
  ...
  return (
    <aside className={`flex flex-col bg-slate-800 text-white h-screen sticky top-0 transition-all duration-300 ${isOpen ? 'w-64' : 'w-20'}`}>
      ...
      <div className={`p-4 border-t border-slate-700 ${!isOpen && 'flex justify-center'}`}>
        {isOpen ? (
          <div className="mb-4">
            <p className="text-sm font-semibold truncate">{user?.name || "Expert"}</p>
