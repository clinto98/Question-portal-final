...
const PayoutPage = () => {
    ...
    useEffect(() => {
        const fetchTransactions = async () => {
            if (selectedUser) {
                setLoading(true);
                try {
                    const response = await axios.get(`${host}/api/admin/payout/transactions/${selectedUser}`, {
                        headers: {
                            'Authorization': `Bearer ${token}`,
                        },
                    });
    ...
    const handlePayout = async (e) => {
        ...
        try {
            ...
            setAmount('');
            setDescription('');
            // Refresh transactions after payout
            const response = await axios.get(`${host}/api/admin/payout/transactions/${selectedUser}`, {
                headers: {
                    'Authorization': `Bearer ${token}`,
                },
    ...
    return (
        <div className="p-6 bg-gray-100 min-h-screen">
            <h1 className="text-3xl font-bold text-gray-800 mb-6">Manage Payouts</h1>

            <div className="bg-white shadow-md rounded-lg p-6 mb-8">
                <h2 className="text-2xl font-semibold text-gray-700 mb-4">Record New Payout</h2>
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                {transactions.map((transaction, index) => (
                                    <tr key={index}>
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{new Date(transaction.timestamp).toLocaleString()}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
