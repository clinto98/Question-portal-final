        ...
        const fetchFilters = async () => {
            ...
            try {
                const [coursesRes, subjectsRes] = await Promise.all([
                    axios.get(`${host}/api/expert/finalized-courses`, {
                        headers: { "Authorization": `Bearer ${token}` },
                    }),
                    axios.get(`${host}/api/expert/finalized-subjects`, {
                        headers: { "Authorization": `Bearer ${token}` },
                    }),
                ]);
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                {questions.map((q) => (
                                    <tr key={q._id}>
                                        ...
                                        <td className="px-6 py-4"><span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">Finalised</span></td>
